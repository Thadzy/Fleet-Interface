...
const GraphEditor: React.FC = () => {
  ...
  // 2. UPDATE THE FUNCTION to use 'setUploading' instead of 'setLoading'
  const handleFileUpload = async (
    ...
    try {
      setUploading(true); // <--- FIXED: Use local state
      ...
      setUploading(false); // <--- FIXED: Use local state
    }
  };
        ...
        <Panel position="top-right" className="m-4">
          <div className="bg-white/90 backdrop-blur border border-slate-200 shadow-lg rounded-xl p-1.5 flex gap-1">
            <div className="flex gap-1 pr-2 border-r border-slate-200 items-center">
              ...
              <button
                onClick={handleDelete} // <--- UPDATED: Uses the new function
              ...
              <button
                onClick={async () => {
                  ...
                  if (success) {
                    ...
                    const { nodes: dbNodes, edges: dbEdges } =
                      await loadGraph();
                    setNodes(dbNodes);
                    setEdges(dbEdges);
                  }
